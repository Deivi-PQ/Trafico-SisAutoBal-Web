...
export async function GrabarUsuario(oUsuario) {
  const url = `${API_URL}api/Usuario/Grabar`;
  console.log("POST:", url, oUsuario);
  ...
  try {
    const response = await axios.post(url, oUsuario, { headers: authHeader() });
...
export async function EliminarUsuario(IDUsuario) {
  const url = `${API_URL}api/Usuario/Eliminar/${IDUsuario}`;
  console.log("GET:", url);
  // Verifica que haya sesión activa
  if (!getjwtToken()) {
    console.error("No hay sesión activa.");
...
export async function RecuperarUsuario(IDUsuario) {
  const url = `${API_URL}api/Usuario/Recuperar/${IDUsuario}`;
  console.log("GET:", url);
  // Verifica que haya sesión activa
  if (!getjwtToken()) {
    console.error("No hay sesión activa.");
...
export async function ListarUsuario(oUsuario) {
  const url = `${API_URL}api/Usuario/Listar`;
  console.log("POST:", url, oUsuario);
  // Verifica que haya sesión activa
  if (!getjwtToken()) {
    console.error("No hay sesión activa.");
