...
import {
  ...
  EliminarT,
  ListarT,
} from "../../../services/Seguridad/Tabla/TablaService";
...
function Tabla() {
  ...
  const [addChildErrors, setAddChildErrors] = useState({
    desTabla: "",
    estado: "",
  });
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const isPadre =
        !rowToDelete.cdPadre ||
        rowToDelete.cdPadre === 0 ||
        rowToDelete.cdPadre === null ||
        rowToDelete.cdPadre === undefined;
      if (isPadre) {
        // Encuentra hijos
        const hijos = tablaData.filter(
          (item) => item.cdPadre === rowToDelete.cdTabla
        );
        for (const hijo of hijos) {
          try {
            ...
            showAlert(
              "No se pudo eliminar el hijo con código " +
                hijo.cdTabla +
                ". Operación cancelada.",
              "error"
            );
        ...
        setTablaData((prev) =>
          prev.filter((item) => item.cdPadre !== rowToDelete.cdTabla)
        ); // Elimina hijos en vista
        await new Promise((res) => setTimeout(res, 100));
      }
      await EliminarT(rowToDelete.cdTabla);
      setTablaData((prev) =>
        prev.filter((item) => item.cdTabla !== rowToDelete.cdTabla)
      ); // Elimina padre
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      setTablaData((prev) =>
        prev.map((item) =>
          item.cdTabla === editData.cdTabla ? { ...item, ...editData } : item
        )
      );
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground">
                Administra las tablas del sistema.
              </p>
            </div>
          </div>
        </div>
        <div className="overflow-x-auto bg-white rounded-2xl shadow-lg p-6 border border-blue-100">
          <div className="flex flex-col sm:flex-row justify-between gap-2 mb-4 items-center">
            ...
            <div className="flex-1 flex items-center">
              {alertMessage && (
                <span
                  className={`transition-all text-sm font-semibold px-3 py-2 rounded-lg shadow-sm border ${
                    alertType === "success"
            ...
            <table className="min-w-full text-sm rounded-xl overflow-hidden text-black">
              <thead>
                <tr className="bg-black text-white">
                  <th className="px-4 py-2 text-left font-bold rounded-tl-xl">
                    Codigo Tabla
                  </th>
                  ...
                  <th className="px-4 py-2 text-center font-bold rounded-tr-xl">
                    Acciones
                  </th>
                </tr>
              </thead>
                ...
                {getPadres().map((padre) => (
                  <React.Fragment key={padre.cdTabla}>
                    <tr className="transition hover:bg-blue-100/60 group border-b border-blue-100 font-semibold">
                      <td className="px-4 py-2 align-middle text-black font-bold">
                        {padre.cdTabla}
                      </td>
                      <td className="px-4 py-2 flex items-center gap-2 align-middle">
                        <Button
                          size="sm"
                          variant="ghost"
                          onClick={() => toggleExpand(padre.cdTabla)}
                          className="rounded-full border border-blue-200 bg-white shadow hover:bg-blue-100 transition"
                        >
                      ...
                      <td className="px-4 py-2 flex gap-2 align-middle justify-center">
                        ...
                        <Button
                          ...
                          icon="plus"
                          className="bg-black text-white border-black hover:bg-gray-800"
                          onClick={() => handleAddChild(padre.cdTabla)}
                        >
                          Agregar Nuevo
                        </Button>
                      </td>
                        ...
                        <td colSpan={5} className="p-0 bg-white">
                          <div className="pl-8 py-2">
                            <table className="w-[95%] mx-auto border border-blue-100 rounded-xl overflow-hidden shadow text-black bg-white">
                              <thead>
                                <tr className="bg-black text-white">
                                  <th className="px-3 py-1 text-left font-semibold rounded-tl">
                                    Codigo Tabla
                                  </th>
                                  ...
                                  <th className="px-3 py-1 text-left font-semibold rounded-tr">
                                    Estado
                                  </th>
                                  <th className="px-4 py-2 text-center font-bold rounded-tr-xl">
                                    Acciones
                              ...
                              <tbody>
                                {getHijos()
                                  .filter((h) => h.cdPadre === padre.cdTabla)
                                  .map((hijo) => (
                                    <tr
                                      key={hijo.cdTabla}
                                      className="hover:bg-blue-50 transition"
                                    >
                                      <td className="px-3 py-1 text-black font-bold">
                                        {hijo.cdTabla}
                                      </td>
              ...
              >
                <form className="space-y-4" onSubmit={handleEditSubmit}>
                  ...
                  {editData.cdPadre !== "" &&
                    editData.cdPadre !== null &&
                    editData.cdPadre !== undefined && (
                      <div>
                        <label className="block text-sm font-semibold mb-1">
                          Código Padre
                        </label>
                ...
                <form className="space-y-4" onSubmit={handleAddChildSubmit}>
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full border rounded px-3 py-2 text-black ${
                        addChildErrors.desTabla ? "border-red-500" : ""
                      }`}
                    />
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={handleAddChildInput}
                      className={`w-full border rounded px-3 py-2 text-black ${
                        addChildErrors.estado ? "border-red-500" : ""
                      }`}
                    >
                      <option value="A">Activo </option>
                      <option value="X">Inactivo</option>
